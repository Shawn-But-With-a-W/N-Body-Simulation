...
function createBody(event) {
    // Another problem with pass by reference and not value
    // TO DO: Actually detect and convert where the mouse is clicing
    ...
    // clientX, clientY are distance from top left corner, practically same as canvas coordinates
    // (event.clientX - canvas.width/2 + translate.x) / zoomLevel
    // -(event.clientY - canvas.height/2 + translate.y) / zoomLevel
}
